---
name: graphify
description: Turn any folder (code, docs, papers, images) into a navigable knowledge graph with community detection, an audit trail, and HTML + JSON + report outputs. Run with /graphify.
disable-model-invocation: true
argument-hint: "[path] [--mode deep|--update|--obsidian|...] or a subcommand (query|path|explain|add)"
---
...
Everything past the main build pipeline is a distinct branch, reached only for a
specific invocation. When the invocation is one of these, follow
[references/subcommands.md](references/subcommands.md):

- `--update` — incremental re-extraction of new/changed files
- `--cluster-only` — rerun clustering on the existing graph
- `/graphify query` — BFS/DFS traversal to answer a question
- `/graphify path` — shortest path between two concepts
- `/graphify explain` — plain-language explanation of one node
- `/graphify add` — fetch a URL into the corpus, then update
- `--watch` — watch a folder and auto-rebuild on change
- git commit hook — auto-rebuild after every commit
- native CLAUDE.md integration — make graphify always-on in a project
