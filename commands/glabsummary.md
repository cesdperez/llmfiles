...
After posting the report here, send a push notification via the `PushNotification` tool without asking: one line, under 200 characters, counting outcomes by suggested action, e.g. `Reviewed 5 MRs: 2 approvable, 2 need changes, 1 thread waiting on you.` Skip it when the queue is empty.
